...
while(<>) {
  ...
  if (/^\s+Instance\s+.*hostname:(\S+),/) {
    $instance="Instance($1)";
    $pipeline = undef;
    $driver = undef;
    $operator = undef;
    $plan->{$fragment}{instances}{$instance}{ip}=$1;
    next;
  }
  ...
  if (/Pipeline\s+\(id=(\d+)\)/) {
    ...
    $driver = undef;
    $operator = undef;
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{id}=join "_", ($fragment, $instance,$pipeline);
    next;
  }

  if (/^\s+PipelineDriver \(id=(\d+)\)/) {
    $driver = "Driver($1)";
    $operator = undef;
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{id}=join "_", ($fragment,$instance,$pipeline,$driver);
    next;
  }
  ...
  if (/\b(MemoryLimit|PeakMemoryUsage|__MAX_OF_PeakMemoryUsage):\s+(.*)/) {
    if (defined($operator)){
      ...
      $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{operators}{$operator}{$1}=$2;
      $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{operators}{$operator}{id}=$operator_id;
    } elsif(defined($pipeline)) {
      $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{$1}=$2;
    } else {
      $plan->{$fragment}{$1}=$2;
      $plan->{$fragment}{id}=$fragment;
    }
  ...
  if (/\b(DegreeOfParallelism|TotalDegreeOfParallelism|PeakDriverQueueSize|__MAX_OF_PeakDriverQueueSize):\s+(\d+(?:(?:\.\d+)\w+)?)/) {
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{$1}=norm_num($2);
    next;
  }

  if (/\b(PendingTime|__MAX_OF_PendingTime):\s+(\S+)/) {
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{$1}=norm_time($2);
    next;
  }
  ...
  if (/\b(PrepareTime|__MAX_OF_PrepareTime|__MIN_OF_PrepareTime|PushTotalTime|SortingTime|__MAX_OF_SortingTime|MergingTime|IOTaskExecTime|__MAX_OF_IOTaskExecTime|IOTaskWaitTime|__MAX_OF_IOTaskWaitTime|ScanTime|__MAX_OF_ScanTime|__MAX_OF_MergingTime|PullTotalTime|CompressTime|SetFinishingTime|BuildHashTableTime|RuntimeFilterBuildTime|CopyRightTableChunkTime|OtherJoinConjunctEvaluateTime|OutputBuildColumnTimer|OutputProbeColumnTimer|OutputTupleColumnTimer|ProbeConjunctEvaluateTime|__MAX_OF_ProbeConjunctEvaluateTime|__MIN_OF_ProbeConjunctEvaluateTime|SearchHashTableTimer|WhereConjunctEvaluateTime|OperatorTotalTime|SetFinishedTime|JoinRuntimeFilterTime|CloseTime)\b:\s+(\S+)/) {
    print qq/$.:$_\n/;
    my $operator_id = join "_", ($fragment, $instance, $pipeline, $driver, $operator);
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{operators}{$operator}{$1}=norm_time($2);
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{operators}{$operator}{id}=$operator_id;
    next;
  }
  ...
  if (/\b(RowsRead|SubmitTaskCount|__MAX_OF_SubmitTaskCount|MorselsCount|__MAX_OF_MorselsCount|IOCounter|__MAX_OF_IOCounter|RawRowsRead|PullChunkNum|PushChunkNum|PullRowNum|DestID|PushRowNum|BlockCacheReadCounter|__MAX_OF_BlockCacheReadCounter|BlockCacheWriteCounter|__MAX_OF_BlockCacheWriteCounter):\s+(\S+)/) {
    my $operator_id = join "_", ($fragment, $instance, $pipeline, $driver, $operator);
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{operators}{$operator}{$1}=norm_num($2);
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{operators}{$operator}{id}=$operator_id;
    next;
  }
  ...
  if (/\b(RuntimeInFilterNum|RuntimeBloomFilterNum):\s+(.*)/) {
    my $operator_id = join "_", ($fragment, $instance, $pipeline, $driver, $operator);
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{operators}{$operator}{$1}=$2+0;
    $plan->{$fragment}{instances}{$instance}{pipelines}{$pipeline}{drivers}{$driver}{operators}{$operator}{id}=$operator_id;
    next;
  }
}
...
my $instances=[map {values %$_} map {$_->{instances}} values @$fragments];
my $pipelines=[map {values %$_}  map {$_->{pipelines}} values @$instances];
my $drivers=[map {values %$_}  map {$_->{drivers}} values @$pipelines];

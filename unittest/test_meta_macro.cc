...
#define M_MODIFIER_DEF_STMT_1(modifier, type, a1) modifier type a1 = type(0);
...
#define DEF_FOOBAR(n, ...) DEF_BINARY_RELATION_ENTRY_SEP_COMMA(1, DEF_FOOBAR_CTOR, n, ##__VA_ARGS__)
...
TEST_F(MetaMacroTest, test_macro_3) {
    ...
    for (int i = 1; i <= 10; ++i) {
        for (int j = 1; j <= i; ++j) {
            int k = ((i - 1) * i / 2 + j) - 1;
            auto& p = pairs[k];
            std::cout << "k=" << k << ", (i=" << i << ", j=" << j << "), (p.first=" << p.first
...
#define IS_ASSIGNABLE(a, ...) DEF_BINARY_RELATION_ENTRY_SEP_NONE(1, IS_ASSIGNABLE_CTOR, a, ##__VA_ARGS__)
#define IS_ASSIGNABLE_R(a, ...) DEF_BINARY_RELATION_ENTRY_SEP_SEMICOLON(0, IS_ASSIGNABLE_CTOR, a, ##__VA_ARGS__)

IS_ASSIGNABLE(int8_t, int16_t, int32_t, int64_t);
IS_ASSIGNABLE_R(double, float, int8_t, int16_t, int32_t, uint8_t, uint16_t, uint32_t);
...
static std::unordered_map<type_enum, type2_enum> global_assignable_table{
        ...
        IS_ASSIGNABLE_ENTRY_R(TYPE2_INT64, TYPE_UINT8, TYPE_INT16, TYPE_UINT16, TYPE_INT32, TYPE_UINT32, TYPE_INT64,
                              TYPE_UINT64),
...
TEST_F(MetaMacroTest, test_assignable_table) {
    ...
    std::vector<type_enum> type1_ints = {TYPE_INT8,  TYPE_UINT8,  TYPE_INT16, TYPE_UINT16,
...
#define IS_BINARY_FUNCTION(varg_pos, a, b, ...) \
    DEF_TERNARY_RELATION_ENTRY_SEP_NONE(IS_BINARY_FUNCTION_CTOR, varg_pos, a, b, ##__VA_ARGS__)
